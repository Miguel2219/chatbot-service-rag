...
**Multi-tenant isolation.** Each bot gets its own ChromaDB collection, named
after its identifier. Isolation is physical rather than a query-time filter, so
one tenant's content cannot surface in another tenant's answers.
...
timeouts — shorter for text, longer for vision, since image processing is
slower — plus a bounded retry count. A slow upstream degrades the response, it
does not hang the service.

**Predictable token cost.** Conversation history sent to the model is truncated
to a fixed number of turns, so prompt size stays bounded no matter how long a
conversation runs.
...
| `SESSION_TTL_SECONDS` | no | Declared but currently unused — session expiry is not implemented |
| `SESSION_MAX_COUNT` | no | Declared but currently unused |
| `SESSION_CLEANUP_INTERVAL_SECONDS` | no | Declared but currently unused |

## Known limitations

**Conversation history is shared per bot, not per user.** RAG engines are cached
by bot id and the session identifier is ignored, so concurrent end users talking
to the same bot share the same conversation context. Scoping engines per session
is the main change this service needs before real multi-user traffic.

**No session eviction.** Cached engines are never released, so memory grows with
the number of distinct bots served. A TTL-based cleanup existed in an earlier
version and was removed; the configuration variables for it are still declared
but unused.
